...
describe('extractHTMLTags', () => {
  test('extracts HTML tags from a string containing HTML', () => {
    expect(
      extractHTMLTags('<p>This is a paragraph.</p> <a href="#">Click here</a>')
    ).toEqual(['<p>', '</p>', '<a href="#">', '</a>']);
  });
...
describe('extractDates', () => {
  // Todo: remove `skip` when test fail issue resolved
  test.skip('extracts dates from a string containing dates', () => {
...
describe('extractPhoneNumbers', () => {
  // Todo: remove `skip` when test fail issue resolved
  test.skip('extracts phone numbers from a string containing phone numbers', () => {
    expect(
      extractPhoneNumbers('Contact us at 123-456-7890 or +1 (987) 654-3210.')
    ).toEqual(['123-456-7890', '+1 (987) 654-3210']);
  });
...
describe('extractIPv4Addresses', () => {
  test('extracts IPv4 addresses from a string containing IPv4 addresses', () => {
    expect(
      extractIPv4Addresses('IP addresses: 192.168.1.1 and 10.0.0.1')
    ).toEqual(['192.168.1.1', '10.0.0.1']);
  });
...
describe('extractIPv6Addresses', () => {
  // Todo: remove `skip` when test fail issue resolved
  test.skip('extracts IPv6 addresses from a string containing IPv6 addresses', () => {
    expect(
      extractIPv6Addresses(
        'IPv6 addresses: 2001:0db8:85a3:0000:0000:8a2e:0370:7334 and fe80::1'
      )
    ).toEqual(['2001:0db8:85a3:0000:0000:8a2e:0370:7334', 'fe80::1']);
  });
...
describe('extractFilePaths', () => {
  // Todo: remove `skip` when test fail issue resolved
  test.skip('extracts file paths from a string containing file paths', () => {
    expect(
      extractFilePaths(
        'File paths: /path/to/file1.txt and C:\\Documents\\file2.docx'
      )
    ).toEqual(['/path/to/file1.txt', 'C:\\Documents\\file2.docx']);
  });
...
describe('extractDomainNames', () => {
  // Todo: remove `skip` when test fail issue resolved
  test.skip('extracts domain names from a string containing domain names', () => {
    expect(
      extractDomainNames('Visit https://www.example.com for more info.')
...
describe('extractJSONStrings', () => {
  test('extracts JSON strings from a string containing JSON strings', () => {
    expect(
      extractJSONStrings('{"key": "value", "nested": {"inner": 42}}')
    ).toEqual(['{"key": "value", "nested": {"inner": 42}}']);
  });
...
describe('removeWhitespace', () => {
  test('removes whitespace from a string', () => {
    expect(removeWhitespace(' Hello  World from STRING  Utils! ')).toEqual(
      'HelloWorldfromSTRINGUtils!'
    );
  });

  test('handles a string with multiple spaces between words', () => {
    expect(removeWhitespace('Multiple    Spaces    Between    Words')).toEqual(
      'MultipleSpacesBetweenWords'
    );
  });
